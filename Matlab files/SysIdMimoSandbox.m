%% Comparing different MIMO identification algorithms with rank reduced noise
...
while counter <= num_loop
    ...
    try     % Used to catch and skip error in case one occurs
        ...
        for trial = trial:nTrials
            ...
            Ts = 1;                 % sample time
            L = 5;                  % number of nodes
            K = 0;                  % number of external excitations (disabled since r is always set to zero)
...
Y_TPR = [avgTPRgranger avgTPRbayesian avgTPRsls];
%
figure(5)
set(gcf,'Position',[100 100 900 500])
...
Y_FPR = [avgFPRgranger avgFPRbayesian avgFPRsls];
%
subplot 122
hold on
bar(X_FPR,Y_FPR)
